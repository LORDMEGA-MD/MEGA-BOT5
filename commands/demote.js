...
async function demoteCommand(sock, chatId, mentionedJids, message) {
    try {
        ...
        const targetsSelf = userToDemote.some(jid => 
            botNumber && jid.split('@')[0].split(':')[0] === botNumber
        );
        if (targetsSelf) {
            await sock.sendMessage(chatId, { 
...
// is admin here. Mirrors the same fix applied to handlePromotionEvent
// in promote.js.
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        try {
            ...
            console.error('[Demote] isAdmin check failed, skipping announcement:', err.message);
