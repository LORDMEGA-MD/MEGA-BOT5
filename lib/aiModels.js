// lib/aiModels.js
// Central registry for ALL AI models — elite/yupra (top priority) + xwolf (37 models) + NVIDIA (18 endpoints)
// Import this wherever you need model definitions, URL builders, or helpers.

const XWOLF_API_BASE = 'https://apis.xwolf.space';
...
const AI_MODELS = {
  ...
  // ── Elite/Yupra text models (external, non-xwolf) ───────────────────────
  // These use their own full URLs and simple query params (no xwolf key needed).
  'elite-copilot':   { name: 'Elite Copilot',  icon: '🚀', endpoint: 'https://eliteprotech-apis.zone.id/copilot', category: 'text', provider: 'elite', param: 'q' },
  'elite-talkai':    { name: 'Elite TalkAI',   icon: '💫', endpoint: 'https://eliteprotech-apis.zone.id/talk-ai', category: 'text', provider: 'elite', param: 'q' },
  'yupra-copilot':   { name: 'Yupra Copilot',  icon: '🌟', endpoint: 'https://api.yupra.my.id/api/ai/copilot',    category: 'text', provider: 'elite', param: 'text' },
  'yupra-gpt5':      { name: 'Yupra GPT-5',    icon: '✨', endpoint: 'https://api.yupra.my.id/api/ai/gpt5',       category: 'text', provider: 'elite', param: 'text' },
};
...
// Elite/Yupra APIs are placed first so they're attempted before the xwolf/NVIDIA chain.
const MODEL_PRIORITY = [
  'elite-copilot', 'elite-talkai', 'yupra-copilot', 'yupra-gpt5',
...
// ── Response extractor ─────────────────────────────────────────────────────
// Works for xwolf, NVIDIA, and elite/yupra API responses.
function extractXWolfResponse(data) {
  ...
  // Primary: NVIDIA / xwolf both use `result`; elite APIs use `text`/`response`/`result`
...
// elite/yupra models already carry their full URL in `endpoint` and use their own param name.
function buildTextUrl(modelKey, query) {
  const model = AI_MODELS[modelKey];
  if (!model) return null;

  if (model.provider === 'elite') {
    const param = model.param || 'q';
    return `${model.endpoint}?${param}=${encodeURIComponent(query)}`;
  }
...
function buildVisionUrl(query, imageBase64) {
